...
// api that shows students without mentor

app.get('/show-student/without-mentor', (req, res) => {
  function WithoutMentor(item) {
    if (item.mentorAssigned == 'nil') return item
  }
...
app.get('/show-mentor/all-students', (req, res) => {
  mentor.map((item) => {
    if (item.mentor_id == req.body.mentorID) res.status(200).json(item.students)
  })
})
        ...
        if (item.students == '') {
          ...
          student.map((ele) => {
            if (ele.student_id == stu.studentID) {
              ele.mentorAssigned = req.body.mentorName
            }
          })
          ...
          item.students.map((ite) => {
            //condition when there are already multiple students to a mentor
